if ( location.hostname == 'sh.fr.to' ){
  var apiurl = 'https://sh.fr.to/source/chat/api.php?'
}else{
  var apiurl = 'http://' + location.hostname + '/source/chat/api.php?'
}
console.log(apiurl);
...
socket.on("join-result", (data)=>{
  ...
  if( data.status ){
    ...
    xhr.open('GET', apiurl + 'logname=utamago&mode=read', true);
    xhr.onload = function () {
      console.log(xhr.responseText);
      var logtext = xhr.responseText.replace(/\r?\n/g, '<br>')
...
$("#frm-quit").addEventListener("submit", (e)=>{
  ...
  if( confirm("本当に退室しますか？") ){
      // 退出ログ保存
      var xhr = new XMLHttpRequest();
      xhr.open('GET', apiurl + 'logtext=' + decodeURIComponent('bot> @' + IAM.name + ' が退出しました') + '&logname=utamago&mode=write', true);
      xhr.onload = function () {
      };
      xhr.send(null);
...
socket.on("member-join", (data)=>{
  if( IAM.is_join ){
    if ( IAM.name == data.name){
      // 入室ログ保存
      var xhr = new XMLHttpRequest();
      xhr.open('GET', apiurl + 'logtext=' + decodeURIComponent('bot> @' + IAM.name + ' が入室しました') + '&logname=utamago&mode=write', true);
      xhr.onload = function () {
      };
      xhr.send(null);
...
function addMessage(msg, is_me=false){
  ...
  // 自分の発言
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('GET', apiurl + 'logtext=' + decodeURIComponent(name + '> ' + msg.text) + '&logname=utamago&mode=write', true);
    xhr.onload = function () {
      console.log(xhr.responseText);
    };
    xhr.send(null);
